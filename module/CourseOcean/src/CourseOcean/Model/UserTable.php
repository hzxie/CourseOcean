...
class UserTable
{
    ...
    public function getUsersUsingFilters($userGroupId, $isInspected, $isApproved, $offset, $limit)
    {
        $resultSet = $this->tableGateway->select(function (Select $select) use ($userGroupId, $isInspected, $isApproved, $offset, $limit) {
            $select->join('co_user_groups', 
                          'co_users.user_group_id = co_user_groups.user_group_id');
            if ( $userGroupId != 0 ) {
                $select->where->equalTo('co_users.user_group_id', $userGroupId);
            }
            if ( $isInspected != -1 ) {
                $select->where->equalTo('is_inspected', $isInspected);
            }
     ...
     * @return 一个ResultSet对象, 包含若干个User对象.
     */
    public function getUsersUsingKeyword($keyword, $offset, $limit)
    {
        $resultSet = $this->tableGateway->select(function (Select $select) use ($keyword, $offset, $limit) {
            $select->join('co_user_groups', 
                          'co_users.user_group_id = co_user_groups.user_group_id');
            $select->where->OR->equalTo('co_users.uid', "$keyword");
            $select->where->OR->like('co_users.username', "%$keyword%");
            $select->where->OR->like('co_users.email', "%$keyword%");
        });
        return $resultSet;
    }
